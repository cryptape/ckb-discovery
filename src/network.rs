...
pub fn addr_to_node_meta(addr: &Multiaddr, network: CKBNetworkType) -> NodeMetaInfo {
    ...
    for protocol in addr.iter() {
        match protocol {
            ...
            Protocol::P2P(p2p) => { peer_id = String::from_utf8_lossy(bs58::encode(p2p).into_vec().as_slice()).to_string() },
...
pub fn meta_info_to_addr(meta: &NodeMetaInfo) -> Vec<Result<Multiaddr, p2p::multiaddr::Error>> {
    meta.addresses.iter().map(|x| {
        match x.address {
            IpAddr::V4(addr) => { format!("/ip4/{}/tcp/{}/p2p/{}", addr.to_string(), x.port, meta.peer_id).parse() },
            IpAddr::V6(addr) => { format!("/ip6/{}/tcp/{}/p2p/{}", addr.to_string(), x.port, meta.peer_id).parse() },
        }
